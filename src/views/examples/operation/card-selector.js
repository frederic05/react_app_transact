...
export const CardSelector = props => {
  ...
  const onMouseEnter = useCallback(event => setCardHover(event.currentTarget.getAttribute('for')), [setCardHover]);
  const onMouseLeave = useCallback(() => setCardHover(''), [setCardHover]);
  const onChange = useCallback(event => props.onChange({
